...
min_cp_pct = 90.0        # soglia above/below (% CP)
merge_gap_sec = 10        # gap max per merge adiacenti
merge_power_diff_pct = 15 # Δ potenza max per merge
min_effort_sec = 120       # post-merge: scarta se troppo corto...
opener_threshold_pct = 1300 # ...a meno che non superi questa % CP
Pipeline:
...
Filter: mostra se duration >= min_effort_sec OR avg_power >= cp * opener_threshold_pct / 100

Obiettivo: Trovare i parametri ottimali per un file FIT reale (CP=260W, ~12000 samples, ~3.5h) confrontando il risultato con effort scelti manualmente tramite visual inspection. Considerare anche l'implementazione di nuovi parametri per la rilevazione e/o il filtraggio.
...
Domanda: Come faccio a calibrare i parametri in modo sistematico? Prima di elaborare una risposta attendi che io condivida i dati grezzi e gli effort "gold" che voglio rilevare di alcune attività... voglio capire quali parametri devo alzare/abbassare o introdurre per avvicinarmi al risultato.
